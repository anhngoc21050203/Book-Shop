...
<?php
  ...
  $sql = "SELECT product.*, nha_xuat_ban.ten_nha_xuat_ban
  FROM product
  INNER JOIN nha_xuat_ban ON product.NXB = nha_xuat_ban.id
  WHERE product.NXB = '$id'";
        ...
        <ol class="breadcrumb">
          ...
          <?php
             if ($result->num_rows > 0) {
              ...
              echo '<li class="breadcrumb-item active" aria-current="page">Thể loại: '.$row['ten_nha_xuat_ban'].'</li>';
              $result->data_seek(0);
             }
          ?>
        </ol>
